...
public:
	...
	addr_t detectorSystem_start_addr = 0x73000000;
 	addr_t detectorSystem_size = 0x01000016;
 	addr_t detectorSystem_end_addr = detectorSystem_start_addr + detectorSystem_size - 1;
	...
	void parse(int argc, char **argv) override {
		...
		assert((mem_end_addr < clint_start_addr || mem_start_addr > detectorSystem_end_addr) &&
...
int sc_main(int argc, char **argv) {
	...
	DetectorSystem detector_system("detector_system");
	...
	bus.ports[12] = new PortMapping(opt.detectorSystem_start_addr,opt.detectorSystem_end_addr);
	...
	bus.isocks[12].bind(detector_system.tsock);
